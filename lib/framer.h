...
namespace sipm4eic
{
  ...
  public:
    framer(std::vector<std::string> filenames, int frame_size = 1024) : _filenames(filenames), _frame_size(frame_size), _verbose(false)
    {
  ...
  bool framer::next_spill()
  {
    ...
    for (const auto filename : _filenames)
    {
      ...
      {
        //std::cout << "[WARNING] Skipping file " << filename << " it does not exist" << std::endl;
        continue;
      }
      ...
      if (!fin || !fin->IsOpen())
      {
        //std::cout << "[WARNING] Skipping file " << filename << ", it could not be opened" << std::endl;
        continue;
      }
      ...
      for (int iev = _next_spill[filename]; iev < nev; ++iev)
      {
        ...
        if (data.is_start_spill())
        {
          has_data = true;
          if (_verbose)
            std::cout << " --- start of spill found: event " << iev << std::endl;
          auto device = data.device;
          auto fifo = data.fifo;
          if (!_part_mask.count(device))
            _part_mask[device] = 0x0;
          _part_mask[device] |= (1 << fifo);
          if (data.coarse_time_clock() == 0xdeadbeef)
          {
            if (!_dead_mask.count(device))
              _dead_mask[device] = 0x0;
        ...
        if (data.is_trigger_tag())
        {
          ...
          auto frame = data.coarse_time_clock() / _frame_size;
          if (_verbose)
            std::cout << " --- trigger hit: device=" << device << " fifo=" << data.fifo << " frame=" << frame << std::endl;
        ...
        {
          if (_verbose)
            std::cout << " --- end of spill found: event " << iev << std::endl;
